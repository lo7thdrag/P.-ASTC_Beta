...
uses
  ...
  uDBAsset_Weapon, tttData, Vcl.ComCtrls, Vcl.ToolWin, System.ImageList, Vcl.ImgList;
...
var
  frmBombPOHGraphic: TfrmBombPOHGraphic;
...
procedure TfrmBombPOHGraphic.FormCreate(Sender: TObject);
begin
  FProbabilityPointList := TList.Create;
  FDeletedProbabilityPointList := TList.Create;
end;

procedure TfrmBombPOHGraphic.FormShow(Sender: TObject);
var
  i : Integer;
  data : TObject;
begin
  with TBomb_Definition(FSelectedProbObj).FData do
  begin
    Caption := 'Range vs. Probability of Hit Curve for ' + Bomb_Identifier;
    dmTTT.GetBombPOHvsRange(Bomb_Index, Ord(FProbabilityGraph), FProbabilityPointList);
  end;
...
procedure TfrmBombPOHGraphic.btnOKClick(Sender: TObject);
begin
...
procedure TfrmBombPOHGraphic.btnApplyClick(Sender: TObject);
var
  i : Integer;
  data : TObject;
begin
...
procedure TfrmBombPOHGraphic.btnCancelClick(Sender: TObject);
begin
  Close;
end;

procedure TfrmBombPOHGraphic.btnDeleteClick(Sender: TObject);
var
  i : Integer;
  data : TObject;
begin
  ...
  if TBomb_Definition(FSelectedPoint).FBomb_POH.List_Index = 0 then
  begin
  ...
  begin
    ...
    if TBomb_Definition(data).FBomb_POH.List_Index = TBomb_Definition(FSelectedPoint).FBomb_POH.List_Index then
    begin
...
procedure TfrmBombPOHGraphic.btnScreenCaptureClick(Sender: TObject);
begin
//  with fCaptureRes do
//  begin
...
procedure TfrmBombPOHGraphic.UpdateProbabilityGraphForm;
begin
...
procedure TfrmBombPOHGraphic.imgGraphMouseDown(Sender: TObject;Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
var
...
//    Exit;
  ...
  begin
    ...
    FProbabilityPointList.Insert(pointIndex, TBomb_Definition(FSelectedPoint));

    btnApply.Enabled := True;
  end;
end;

procedure TfrmBombPOHGraphic.imgGraphMouseMove(Sender: TObject; Shift: TShiftState; X, Y: Integer);
var
  aspect, probability : Double;
begin
  if (X < FGrid.Left) or (X > FGrid.Right) or (Y < FGrid.Top) or (Y > FGrid.Bottom) then
...
procedure TfrmBombPOHGraphic.imgGraphMouseUp(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
var
...
begin
  ...
  begin
    ...
    begin
      ...
      if TBomb_Definition(data).FBomb_POH.Range = TBomb_Definition(FSelectedPoint).FBomb_POH.Range then
      begin
        FProbabilityPointList.Delete(i);
        Break;
      end;
...
procedure TfrmBombPOHGraphic.edtAspectMaxKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
  begin
...
procedure TfrmBombPOHGraphic.edtAspectMinKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
  begin
...
procedure TfrmBombPOHGraphic.edtProbabilityMaxKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
  begin
...
procedure TfrmBombPOHGraphic.edtProbabilityMinKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
  begin
...
procedure TfrmBombPOHGraphic.DrawGrid;
var
...
procedure TfrmBombPOHGraphic.DrawPoint;
var
...
procedure TfrmBombPOHGraphic.ConvertPositionToValue(const aX, aY: Integer; var aAspect, aProbability: Double);
var
  posPercent : Double;
begin
...
procedure TfrmBombPOHGraphic.ConvertValueToPosition(const aAspect, aProbability: Double; var aX, aY: Integer);
var
  posPercent : Double;
begin
...
function TfrmBombPOHGraphic.GetPointPosition(const aAspect, aProbability: Double): Integer;
var
  i : Integer;
  data : TObject;
begin
