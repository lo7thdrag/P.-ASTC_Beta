...
type
  ...
  TfrmMinePODGraphic = class(TForm)
    il1: TImageList;
...
var
  frmMinePODGraphic: TfrmMinePODGraphic;
...
procedure TfrmMinePODGraphic.FormCreate(Sender: TObject);
begin
...
begin
  ...
  begin
    ...
    dmTTT.GetMinePODvsRange(Mine_Index, Ord(FProbabilityGraph), FProbabilityPointList);
  end;
...
procedure TfrmMinePODGraphic.btnOKClick(Sender: TObject);
begin
...
procedure TfrmMinePODGraphic.btnApplyClick(Sender: TObject);
var
  i : Integer;
  data : TObject;
begin
...
procedure TfrmMinePODGraphic.btnCancelClick(Sender: TObject);
begin
  Close;
end;

procedure TfrmMinePODGraphic.btnDeleteClick(Sender: TObject);
var
  i : Integer;
  data : TObject;
begin
  ...
  if TMine_On_Board(FSelectedPoint).FMine_POD.List_Index = 0 then
  begin
  ...
  begin
    ...
    if TMine_On_Board(data).FMine_POD.List_Index = TMine_On_Board(FSelectedPoint).FMine_POD.List_Index then
    begin
...
procedure TfrmMinePODGraphic.btnScreenCaptureClick(Sender: TObject);
begin
//  with fCaptureRes do
//  begin
...
procedure TfrmMinePODGraphic.UpdateProbabilityGraphForm;
begin
  ...
  begin
    ...
    begin
      ...
      lblSelectedProbability.Caption := FormatFloat('0.00', Prob_of_Detonation * 100);
    end;
  end
  else
  begin
...
procedure TfrmMinePODGraphic.imgGraphMouseDown(Sender: TObject;Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
var
...
//    Exit;
  ...
  begin
    ...
    begin
      ...
      with TMine_On_Board(data).FMine_POD do
      begin
        if (Range < FAspectMin) or (Range > FAspectMax) or ((Prob_of_Detonation * 100) < FProbabilityMin) or ((Prob_of_Detonation * 100) > FProbabilityMax) then
          Continue;
  ...
  begin
    ...
    with TMine_On_Board(FSelectedPoint).FMine_POD do
    begin
      Mine_Index := TMine_On_Board(FSelectedProbObj).FData.Mine_Index;
      Target_Type := Ord(FProbabilityGraph);
      Prob_of_Detonation := probability / 100;
      Range := aspect;
    end;

    FProbabilityPointList.Insert(pointIndex, TMine_On_Board(FSelectedPoint));

    btnApply.Enabled := True;
  end;
end;

procedure TfrmMinePODGraphic.imgGraphMouseMove(Sender: TObject; Shift: TShiftState; X, Y: Integer);
var
  aspect, probability : Double;
begin
  if (X < FGrid.Left) or (X > FGrid.Right) or (Y < FGrid.Top) or (Y > FGrid.Bottom) then
...
procedure TfrmMinePODGraphic.imgGraphMouseUp(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
var
...
begin
  ...
  begin
    ...
    begin
      ...
      if TMine_On_Board(data).FMine_POD.Range = TMine_On_Board(FSelectedPoint).FMine_POD.Range then
      begin
        FProbabilityPointList.Delete(i);
        Break;
      end;
...
procedure TfrmMinePODGraphic.edtAspectMaxKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
  begin
...
procedure TfrmMinePODGraphic.edtAspectMinKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
  begin
...
procedure TfrmMinePODGraphic.edtProbabilityMaxKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
  begin
...
procedure TfrmMinePODGraphic.edtProbabilityMinKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
  begin
...
procedure TfrmMinePODGraphic.DrawGrid;
var
...
procedure TfrmMinePODGraphic.DrawPoint;
var
...
procedure TfrmMinePODGraphic.ConvertPositionToValue(const aX, aY: Integer; var aAspect, aProbability: Double);
var
  posPercent : Double;
begin
...
procedure TfrmMinePODGraphic.ConvertValueToPosition(const aAspect, aProbability: Double; var aX, aY: Integer);
var
  posPercent : Double;
begin
...
function TfrmMinePODGraphic.GetPointPosition(const aAspect, aProbability: Double): Integer;
var
  i : Integer;
  data : TObject;
begin
