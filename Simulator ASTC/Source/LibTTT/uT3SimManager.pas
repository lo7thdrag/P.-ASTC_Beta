...
uses
  MapXLib_TLB, Classes, SysUtils, System.Win.Registry, System.Variants, Graphics,
  uThreadTimer, WbemScripting_TLB, uVirtualTime, uLibSettingTTT,
  uDBGame_Defaults, uDBScenario, uDBAsset_Weapon, uDBAssetObject, uSimManager,
  uSimObjects, uDBAsset_GameEnvironment, uDBRainfall, uDBCloud_Effect,
  uDBDefault_Var, tttData, uGameData_TTT, uT3Unit, uT3UnitContainer,
  uT3EventManager, uT3Sensor, uT3Weapon, uT3counterMeasure, uT3Vehicle,
  uT3Listener, uDBAsset_Scripted, uWaypoint, uDBAsset_Radar, uDBAsset_Sonar,
  uDBAsset_Sensor, uT3DataLink, uT3MissileDetail, uT3MissileEnvironment,
  newClassASTT, uCoordConvertor, uT3HybridOnVehicle, uT3HybridMissile,
  uDBAsset_MotionCharacteristics, uT3DefensiveJammer, uT3DatalinkManager,
  uSteppers, Dialogs, uDBFormation, uDrawLogisticTemplate, uDrawStrategi,
  uDrawOverlay, uDBAsset_Embark_Library, uNetBaseSocket, uBrigadePersonel,
  uT3GroupVehicle, uDBCubicles;
...
type
  ...
  private
    ...
    TempIntelijen, TempPangkalan, TempLogistic: TList;
...
//    procedure CreateTT3TorpedoInstance(Pi : TPlatform_Instance);
  ...
  protected
    ...
    FOnReplayStr, FOnLogInitStr: TGetStrProc;
...
//    procedure CheckAllMineBomb(aDeltaMs : Double); virtual;
    ...
    function LaunchNonRealtimeRuntimePlatform(const rec: TRecCmd_LaunchNonRP): TT3PlatformInstance; virtual;
    ...
    function FindT3DeviceByID(classT: TClass; Parent: TSimObject; const id: integer): TT3DeviceUnit;
    function FindDeviceByID(id: integer): TT3Unit;
    function FindNearestTrack(const px, py: double; const maxDist: integer): TSimObject;
    function FindNearestTrackExcept(const px, py: double; const maxDist: integer; const id: Integer): TSimObject;
    ...
    function GetPersonel(list: TList; identifier: string; var Personel: TPersonel; level: Integer; var idx: Integer): Boolean;
  ...
  public
    ...
    procedure CopyDataPlatformInstance(const piInput: TPlatform_Instance; var piOutput: TPlatform_Instance);
...
//    procedure netRecv_CmdTorpedoSync(apRec: PAnsiChar; aSize: Word);virtual;
    ...
    procedure CreateOverlayShapeDynamic(rec: TRecCmd_OverlayDynamicShape); virtual;
    {$ENDREGION}
...
uses
  uDBAsset_Vehicle, uT3Missile, uT3Gun, uT3Radar, uT3Sonar, uT3Sonobuoy,
  uT3visual, uT3OtherSensor, uT3Common, uT3Torpedo, uT3Mine, uT3Bomb, Windows,
  forms, uGameSetting, uDBAssets_SubAreaEnviroDefinition,
...
function TT3SimManager.FindDeviceByIDAndName(const id: Integer; const Name: string): TT3Unit;
var
  dvc: TT3Unit;
  f: boolean;
  I, c: Integer;
  ...
  case r^.OrderDL of
    ...
    odlActivate:
      begin
        ...
        if DataLinkManager.IsDLPlayed(r^.DataLinkID) then
        begin
    ...
    odlPUStatus:
      DataLinkManager.SetParticipatingUnitStatus(r^.DataLinkID, aPlatform, r^.PUStatus);
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            if (TT3Vehicle(puPlatform).EMCON_HFDataLinkVehicle <> EmconHFDatalinkOn) and (TT3Vehicle(puPlatform).EMCON_UHFVHFDataLinkVehicle <> EmconUHFVHFDatalinkOn) then
            begin
...
procedure TT3SimManager.netRecv_CMD_DataLink_RemoveTrack(apRec: PAnsiChar; aSize: word);
begin
...
begin
  ...
  begin
    with TPropertyEventListener(Listener) do
    begin
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if ((TT3Sensor(dev).OperationalStatus = sopOn) or (TT3Sensor(dev).OperationalStatus = sopActive) or (TT3Sensor(dev).OperationalStatus = sopDeployed) or     //17042012 mk
          //(TT3Sensor(dev).OperationalStatus = sopStowed)    or     //17042012 mk
          (TT3Sensor(dev).OperationalStatus = sopPassive)) and (TT3Sensor(dev).OperationalStatus <> sopDamage) then
...
begin
  ...
  begin
    ...
    range := CalcRange(sensor.PosX, sensor.PosY, pfObject.getPositionX, pfObject.getPositionY);
...
//                and (isInsideOtherMeasurement);
    ...
    allState := isInRange and (not isInsideBlindZone) and isInsideVerticalCov and (sensor.EmconOperationalStatus <> EmconOn);
...
procedure TT3SimManager.CopyChaffLauncherProp(const rec: TRecCmd_LaunchChaffState);
var
  i: Integer;
...
begin
  ...
  begin
    ...
    begin
      ...
      TT3ChaffLauncher(ChaffLaunchers.Items[i]).Angle := TT3ChaffLauncher(ChaffLaunchers.Items[rec.LauncherID]).Angle;

      TT3ChaffLauncher(ChaffLaunchers.Items[i]).BloomRange := TT3ChaffLauncher(ChaffLaunchers.Items[rec.LauncherID]).BloomRange;
      TT3ChaffLauncher(ChaffLaunchers.Items[i]).BloomAltitude := TT3ChaffLauncher(ChaffLaunchers.Items[rec.LauncherID]).BloomAltitude;
      ;
      TT3ChaffLauncher(ChaffLaunchers.Items[i]).SalvoSize := TT3ChaffLauncher(ChaffLaunchers.Items[rec.LauncherID]).SalvoSize;
      ;
      TT3ChaffLauncher(ChaffLaunchers.Items[i]).Delay := TT3ChaffLauncher(ChaffLaunchers.Items[rec.LauncherID]).Delay;
      ;
    end;
  end;
end;
...
procedure TT3SimManager.CounterMeasureSet(aList: TList; aClass: TClass; aParent: TT3Vehicle);
var
  j: integer;
  ecm: TObject;
begin
  ...
  begin
    ...
    if aClass = TT3AcousticDecoyOnVehicle then
      ecm := TT3AcousticDecoyOnVehicle.Create
    else if aClass = TT3ChaffOnVehicle then
      ecm := TT3ChaffOnVehicle.Create
    else if aClass = TT3FloatingDecoyOnVehicle then
      ecm := TT3FloatingDecoyOnVehicle.Create
    else if aClass = TT3InfraredDecoyOnVehicle then
      ecm := TT3InfraredDecoyOnVehicle.Create
    else if aClass = TT3TowedJammerOnVehicle then
      ecm := TT3TowedJammerOnVehicle.Create
    else if aClass = TT3DefensiveJammerOnVehicle then
      ecm := TT3DefensiveJammerOnVehicle.Create
    else if aClass = TT3ChaffLauncher then
      ecm := TT3ChaffLauncher.Create
    else if aClass = TT3AirBubbleOnVehicle then
      ecm := TT3AirBubbleOnVehicle.Create;
    if aClass = TT3RadarNoiseJammerOnVehicle then
      ecm := TT3RadarNoiseJammerOnVehicle.Create;   //jammer    //nando
    ...
    CORD_ID_Deploy_Float:
      begin
        ...
        if Assigned(decoyOnVeh) and (TT3FloatingDecoyOnVehicle(decoyOnVeh).Quantity > 0) then
          with TT3FloatingDecoyOnVehicle(decoyOnVeh) do
          begin
            decoy := Deploy;
    ...
    CORD_ID_Deploy_IR:
      begin
        ...
        if Assigned(decoyOnVeh) and (TT3InfraredDecoyOnVehicle(decoyOnVeh).Quantity > 0) then
          with TT3InfraredDecoyOnVehicle(decoyOnVeh) do
          begin
            decoy := Deploy;
            decoy.GroupIndex := pi.GroupIndex;
...
//  end;
  ...
  if (FElapseTimeToGo <= 0) or (FGotoTimeIndex = -1) then
  begin
    FGotoThread.Enabled := False;
    GameSPEED := 0.0;
...
procedure TT3SimManager.GetFuelConsBySpeed(aVehicle: TT3Vehicle; aSpeed: Double; var afuelCons: Double);
begin
  if aSpeed = aVehicle.UnitMotion.FData.Min_Ground_Speed then
    afuelCons := aVehicle.UnitMotion.FData.Min_Speed_Fuel_Consume
  else if (aSpeed > aVehicle.UnitMotion.FData.Min_Ground_Speed) and (aSpeed < aVehicle.UnitMotion.FData.High_Ground_Speed) then
    afuelCons := aVehicle.UnitMotion.FData.Cruise_Speed_Fuel_Consume
  else if (aSpeed >= aVehicle.UnitMotion.FData.High_Ground_Speed) and (aSpeed < aVehicle.UnitMotion.FData.Max_Ground_Speed) then
...
function TT3SimManager.GetPersonel(list: TList; identifier: string; var Personel: TPersonel; level: Integer; var idx: Integer): Boolean;
var
  I: Integer;
begin
  Result := False;
  ...
  begin
    case level of
      0:
        Personel := TBrigade(list.Items[I]);
      1:
        Personel := TBatalyon(list.Items[I]);
      2:
        Personel := TKompi(list.Items[I]);
      3:
        Personel := TPlatoon(list.Items[I]);
      4:
        Personel := TGroup(list.Items[I]);
    end;
...
begin
  ...
  begin
    ...
    else if FGotoTimeIndex = 1000 then
    begin
...
begin
  ...
  begin
    ...
    begin
      hostShipTemp.SumCurrentWeight := hostShipTemp.CurrentHanggarWeight + hostShipTemp.CurrentOnBoardWeight + hostShipTemp.CurrentPersonelWeight + hostShipTemp.CurrentHullWeight;
    end;
  end;
...
begin
  ...
  begin
    ...
    case ref_point.FData.Track_Type of
      0:
        rec.PointType := nrpBearing;
      1:
        rec.PointType := nrpSpecial;
      2:
        rec.PointType := nrpPoint;
      3:
        rec.PointType := nrpArea;
    end;
    ...
    begin
      ...
      case StrToInt(domain) of
        0:
          rec.TrackDomain := vhdAir;
        1:
          rec.TrackDomain := vhdSurface;
        2:
          rec.TrackDomain := vhdSubsurface;
        3:
          rec.TrackDomain := vhdLand;
      end;
...
//          if v_platform = p then
      ...
      begin
        ...
        if status = 1 then
        begin
          if Assigned(leader_platform) then
          begin
      ...
      begin
        case identity of
          ...
          1:
            bmp_name := pctBuoy;
          2:
            bmp_name := pctDataLinkReferencePoint;
          3:
            bmp_name := pctDistressedVessel;
          4:
            bmp_name := pctEsmEcmFix;
          5:
            bmp_name := pctGroundZero;
          6:
            bmp_name := pctManInWater;
          7:
            bmp_name := pctMaritimeHeadquarters;
          8:
            bmp_name := pctMineHazard;
          9:
            bmp_name := pctNavigationalHazard;
          10:
            bmp_name := pctOilRig;
          11:
            bmp_name := pctStation;
          12:
            bmp_name := pctTacticalGridorigin;
          13:
            bmp_name := pctAirGeneral;
          14:
            bmp_name := pctAirborneEarlyWarning;
          15:
            bmp_name := pctBullseye;
          16:
            bmp_name := pctCombatAirPatrol;
          17:
            bmp_name := pctDitchedAirCraft;
          18:
            bmp_name := pctGate;
          19:
            bmp_name := pctAswGeneral;
          20:
            bmp_name := pctBriefContact;
          21:
            bmp_name := pctDatum;
          22:
            bmp_name := pctKingpin;
          23:
            bmp_name := pctRiser;
          24:
            bmp_name := pctSearchCentre;
          25:
            bmp_name := pctSinker;
          26:
            bmp_name := pctWeaponEntryPoint;
          27:
            bmp_name := pctWreck;
        end;
        bmp_name := bmp_name;
      ...
      begin
        ...
        case domain of
          0:
            domain_name := vhdAir;
          1:
            domain_name := vhdSurface;
          2:
            domain_name := vhdSubsurface;
          3:
            domain_name := vhdLand;
        end;
        ...
        case identity of
          0:
            bmp_name := getDetectedSymbolFileName(domain_name, piPending);
          1:
            bmp_name := getDetectedSymbolFileName(domain_name, piUnknown);
          2:
            bmp_name := getDetectedSymbolFileName(domain_name, piAssumedFriend);
          3:
            bmp_name := getDetectedSymbolFileName(domain_name, piFriend);
          4:
            bmp_name := getDetectedSymbolFileName(domain_name, piNeutral);
          5:
            bmp_name := getDetectedSymbolFileName(domain_name, piSuspect);
          6:
            bmp_name := getDetectedSymbolFileName(domain_name, piHostile);
        end;
      end;
  end;
...
function TT3SimManager.getNameReferencePointInt(tipe, domain, identity: Integer): Char;
var
  bmp_name: Char;
  domain_name: Integer;
begin
      ...
      begin
        case identity of
          ...
          1:
            bmp_name := pciBuoy;
          2:
            bmp_name := pciDataLinkReferencePoint;
          3:
            bmp_name := pciDistressedVessel;
          4:
            bmp_name := pciEsmEcmFix;
          5:
            bmp_name := pciGroundZero;
          6:
            bmp_name := pciManInWater;
          7:
            bmp_name := pciMaritimeHeadquarters;
          8:
            bmp_name := pciMineHazard;
          9:
            bmp_name := pciNavigationalHazard;
          10:
            bmp_name := pciOilRig;
          11:
            bmp_name := pciStation;
          12:
            bmp_name := pciTacticalGridorigin;
          13:
            bmp_name := pciAirGeneral;
          14:
            bmp_name := pciAirborneEarlyWarning;
          15:
            bmp_name := pciBullseye;
          16:
            bmp_name := pciCombatAirPatrol;
          17:
            bmp_name := pciDitchedAirCraft;
          18:
            bmp_name := pciGate;
          19:
            bmp_name := pciAswGeneral;
          20:
            bmp_name := pciBriefContact;
          21:
            bmp_name := pciDatum;
          22:
            bmp_name := pciKingpin;
          23:
            bmp_name := pciRiser;
          24:
            bmp_name := pciSearchCentre;
          25:
            bmp_name := pciSinker;
          26:
            bmp_name := pciWeaponEntryPoint;
          27:
            bmp_name := pciWreck;
        end;
        bmp_name := bmp_name;
...
function TT3SimManager.FindT3DeviceByID(classT: TClass; Parent: TSimObject; const id: integer): TT3DeviceUnit;
var
  i, c: Integer;
  dev: TT3DeviceUnit;
  f: boolean;
...
function TT3SimManager.FindT3PlatformByID(const id: integer): TT3PlatformInstance;
var
  i, c: Integer;
  pi: TT3PlatformInstance;
  f: boolean;
...
begin
  ...
  begin
    ...
    case pi.FreeMe of
      True:
        S := 'FreeMe Platform : True';
      False:
        S := 'FreeMe Platform : False';
    end;
    ...
    if Assigned(OnLogTemporary) then
      OnLogTemporary('Platform Name : ' + pi.InstanceName + ' , ' + S)
  end;
end;
...
function TT3SimManager.FindNearestTrack(const px, py: double; const maxDist: integer): TSimObject;
var
  i: Integer;
  pi: TSimObject;
  rr: Integer;
...
function TT3SimManager.FindNearestTrackExcept(const px, py: double; const maxDist: integer; const id: Integer): TSimObject;
var
  i: Integer;
  pi: TSimObject;
  rr: Integer;
...
function TT3SimManager.FindNonRealPlatformByID(const id, group: integer): TT3PlatformInstance;
var
  i, c: Integer;
  pi: TT3PlatformInstance;
  f: boolean;
...
function TT3SimManager.FindWeaponByIDAndName(const id: Integer; const Name: string): TT3Weapon;
var
  wpn: TT3Weapon;
  f: boolean;
  I, c: Integer;
...
begin
  ...
  begin
    ...
    if embark = nil then
      Continue;
    if embark.IndexEmbark <> r.IndexEmbark then
      Continue;
    if embark.getQueueLandCount <= 0 then
      Exit;

    for J := embark.getQueueLandCount - 1 downto 0 do
    begin
      queueEmb := embark.getQueueLandItem(J);
      if queueEmb = nil then
        Continue;
...
begin
  ...
  if (rec.Hosted_Index < 0) or (rec.Hosted_Index >= papi.EmbarkedVehicles.Count) then
    exit;
...
//    Exit;
  ...
  begin
    ...
    if hostPlatform.TempReadying = -1 then
      result := hostPlatform.AddQueue(rec.InitSpeed, rec.InitCourse, TrckID, rec.LaunchName, pi.Vehicle.FData.Alert_State_Time, rec.EmbarkPlatformID)//.Readying_Time)
    else
      result := hostPlatform.AddQueue(rec.InitSpeed, rec.InitCourse, TrckID, rec.LaunchName, hostPlatform.TempReadying, rec.EmbarkPlatformID);
            ...
            begin
              ...
              begin
                case Round(rec^.OrderParam) of
                  ...
                  1:
                    TT3Vehicle(p).StationDrift := True;
                end;
...
//              LoadFormation(TT3Vehicle(p),  0);
    ...
    CORD_ID_ACTIVATION:
      ...
      // release dari formasi jika selain vgtformation
        ...
        case rec^.OrderType of
          Byte(vgtStraightLine):
            TVehicle_Definition(p.UnitDefinition).GuidanceType := vgtStraightLine;
          Byte(vgtHelm):
            TVehicle_Definition(p.UnitDefinition).GuidanceType := vgtHelm;
          Byte(vgtCircle):
            begin
          ...
          Byte(vgtZigzag):
            begin
              ...
              else if rec^.OrderParam = 2 then
                TT3Vehicle(p).SetZigzagMovementLong
              else if rec^.OrderParam = 3 then
          ...
          Byte(vgtEvasion):
            TVehicle_Definition(p.UnitDefinition).GuidanceType := vgtEvasion;
          Byte(vgtWaypoint):
            begin
          ...
          Byte(vgtOutrun):
            TVehicle_Definition(p.UnitDefinition).GuidanceType := vgtOutrun;
          Byte(vgtEngagement):
            TVehicle_Definition(p.UnitDefinition).GuidanceType := vgtEngagement;
          Byte(vgtShadow):
            TVehicle_Definition(p.UnitDefinition).GuidanceType := vgtShadow;
          Byte(vgtReturnToBase):
            TVehicle_Definition(p.UnitDefinition).GuidanceType := vgtReturnToBase;
          Byte(vgtFormation):
            begin
...
procedure TT3SimManager.netRecv_Platform_PersonelMove(apRec: PAnsiChar; aSize: Word);
var
...
begin
  ...
  case rec.OrderID of
    CORD_ID_SD_SETMODE, CORD_ID_SD_SETMANMODE, CORD_ID_SD_SETSPOTNUMBER, CORD_ID_SD_SETBEARING, CORD_ID_SD_SETTRACK:
      begin
        jammerObject := nil;

        jammerOwner := FindT3PlatformByID(rec.JammerObjectOwnerID);
        if Assigned(jammerOwner) then
          jammerObject := FindT3DeviceByID(TT3DefensiveJammerOnVehicle, TSimObject(jammerOwner), rec.JammerObjectID);
        ...
        begin
          ...
          case Byte(rec^.JammedObjectType) of
            Byte(jotMissile):
              TargetIndex := rec^.JammedObjectID;
            Byte(jotFCR):
              TargetIndex := rec^.JammedObjectOwnerID;
          end;
        end;
      end;
  end;
...
function TT3SimManager.CreateGroupVehicle(rec: TRecCmdMultiMode): T3GroupVehicle;
var
  p: TT3PlatformInstance;
  i, j: Integer;
  aObj: TObject;
  ...
  case rec.OrderType of
    CORD_TYPE_WEAPON:
      begin
        ...
        //MISSILE
          else if (wpn is TT3MissilesOnVehicle) then
          begin
        ...
        //BOMB
          else if (wpn is TT3BombONVehicle) then
          begin
        ...
        //MINE
          else if (wpn is TT3MineOnVehicle) then
          begin
        ...
        //Hybrid
          else if (wpn is TT3HybridOnVehicle) then
          begin
        ...
        //GUN
          ...
          //if TT3GunOnVehicle(ve.Weapons.Items[i]).GunDefinition.FPoint.FData.Point_Effect_Index = rec^.IDUnit then
            ...
            begin
              ...
              case stats of
                0:
                  TT3GunOnVehicle(ve.Weapons.Items[I]).WeaponStatus := wsAvailable;
                1:
                  TT3GunOnVehicle(ve.Weapons.Items[I]).WeaponStatus := wsUnavailable;
                2:
                  TT3GunOnVehicle(ve.Weapons.Items[I]).WeaponStatus := wsDamaged;
                3:
                  TT3GunOnVehicle(ve.Weapons.Items[I]).WeaponStatus := wsTooHigh;
              end;
            end;
          end
        //MISSILE
          ...
          begin
            ...
            begin
              ...
              case stats of
                0:
                  TT3MissilesOnVehicle(ve.Weapons.Items[I]).WeaponStatus := wsAvailable;
                1:
                  TT3MissilesOnVehicle(ve.Weapons.Items[I]).WeaponStatus := wsUnavailable;
                2:
                  TT3MissilesOnVehicle(ve.Weapons.Items[I]).WeaponStatus := wsDamaged;
                3:
                  TT3MissilesOnVehicle(ve.Weapons.Items[I]).WeaponStatus := wsTooHigh;
              end;
            end;
          end
        //TORPEDO
          ...
          begin
            ...
            begin
              ...
              case stats of
                0:
                  TT3TorpedoesOnVehicle(ve.Weapons.Items[I]).WeaponStatus := wsAvailable;
                1:
                  TT3TorpedoesOnVehicle(ve.Weapons.Items[I]).WeaponStatus := wsUnavailable;
                2:
                  TT3TorpedoesOnVehicle(ve.Weapons.Items[I]).WeaponStatus := wsDamaged;
                3:
                  TT3TorpedoesOnVehicle(ve.Weapons.Items[I]).WeaponStatus := wsTooHigh;
              end;
            end;
          end
        //BOMB
          ...
          begin
            ...
            begin
              ...
              case stats of
                0:
                  TT3BombONVehicle(ve.Weapons.Items[I]).WeaponStatus := wsAvailable;
                1:
                  TT3BombONVehicle(ve.Weapons.Items[I]).WeaponStatus := wsUnavailable;
                2:
                  TT3BombONVehicle(ve.Weapons.Items[I]).WeaponStatus := wsDamaged;
                3:
                  TT3BombONVehicle(ve.Weapons.Items[I]).WeaponStatus := wsTooHigh;
              end;
            end;
          end
        //MINE
          ...
          begin
            ...
            begin
              ...
              case stats of
                0:
                  TT3MineOnVehicle(ve.Weapons.Items[I]).WeaponStatus := wsAvailable;
                1:
                  TT3MineOnVehicle(ve.Weapons.Items[I]).WeaponStatus := wsUnavailable;
                2:
                  TT3MineOnVehicle(ve.Weapons.Items[I]).WeaponStatus := wsDamaged;
                3:
                  TT3MineOnVehicle(ve.Weapons.Items[I]).WeaponStatus := wsTooHigh;
              end;
            end;
          end;
        end;
...
procedure TT3SimManager.netRecv_CmdRadarNoiseJammer(apRec: PAnsiChar; aSize: word);
var
...
begin
  ...
  { find jammer }
  if Assigned(jammerOwner) then
    jammerObject := FindT3DeviceByID(TT3RadarNoiseJammerOnVehicle, TSimObject(jammerOwner), rec.JammerID);

  if not Assigned(jammerObject) then
    Exit;

  case rec.OrderID of
    CORD_ID_RN_SETJAMSTATUS, CORD_ID_RN_SETJAMMODE, CORD_ID_RN_SETJAMSPOTNUMB, CORD_ID_RN_SETJAMBEARING, CORD_ID_RN_SETJAMCENTFREQ, CORD_ID_RN_SETJAMBANDWIDTH, CORD_ID_RN_SETJAMTRACK:
      begin
        ...
        begin
          ...
          case rec^.JammerMode of
            0:
              JamTargeting := ettNJBarrage;
            1:
              JamTargeting := ettNJTrack;
            2:
              JamTargeting := ettNJSpotNum;
            3:
              JamTargeting := ettNJFreq;
          end;
...
procedure TT3SimManager.netRecv_CmdBombSyncPanelWeapon(apRec: PAnsiChar; aSize: word);
var
...
begin
  ...
  begin
    ...
    begin
      datalink.EmconOperationalStatus := TEMCON_OperasionalStatus(byte(r^.OrderParam));
    ...
    begin
      ...
      begin
        TT3Radar(sensor).EmconOperationalStatus := TEMCON_OperasionalStatus(byte(r^.OrderParam));

        if (TT3Radar(sensor).EmconOperationalStatus = EmconOn) and (TT3Radar(sensor).RadarDefinition.FDef.Num_FCR_Channels > 0) then
        begin
          if TT3Radar(sensor).AssignedTrack <> nil then
          begin
            if (TT3Radar(sensor).AssignedTrack.Count <= 0) or (TT3Radar(sensor).AssignedTrack.Count > TT3Radar(sensor).RadarDefinition.FDef.Num_FCR_Channels) then
            begin
...
//            and (not(sensor is TT3SonobuoyOnVehicle)) then
        if (sensor is TT3Sonar) or (sensor is TT3IFFSensor) or (sensor is TT3ESMSensor) or (sensor is TT3EOSensor) then
        begin
    ...
    //------------------------- EMCON SENSOR ----------------------------//
      ...
      begin
        ...
        begin
          ...
          if (TT3Radar(obj).EmconOperationalStatus = EmconOn) and (TT3Radar(obj).RadarDefinition.FDef.Num_FCR_Channels > 0) then
          begin
            if TT3Radar(obj).AssignedTrack <> nil then
            begin
              if (TT3Radar(obj).AssignedTrack.Count <= 0) or (TT3Radar(obj).AssignedTrack.Count > TT3Radar(obj).RadarDefinition.FDef.Num_FCR_Channels) then
              begin
...
procedure TT3SimManager.netRecv_CmdTorpedoProperty(apRec: PAnsiChar; aSize: word);
var
...
procedure TT3SimManager.netRecv_CmdTorpedoSyncPanelWeapon(apRec: PAnsiChar; aSize: word);
var
...
begin
  ...
  begin
    ...
    case r^.cmd of
      0: //Add Waypoint
        begin
          PointBOLW := TPointVehicleMissileBOLW.Create(VehicleWeapon.BOLW_PointX, VehicleWeapon.BOLW_PointY, r^.BOLWIndex, r^.BOLWLauncherIndex, r^.PlatformID);
        ...
        begin
          ...
          begin
            ...
            case OriginalSaved.Action of
              waMove:
                begin
                  ...
                  begin
                    ...
                    if (OriginalSaved.ShipID = r^.PlatformID) and (OriginalSaved.PointIndex = PointBOLW.IndexPoint) and (OriginalSaved.PointLauncherIndex = PointBOLW.LauncherIndex) then
                    begin
              ...
              waDelete:
                begin
                  PointBOLW := TPointVehicleMissileBOLW.Create(OriginalSaved.PointX, OriginalSaved.PointY, OriginalSaved.PointIndex, OriginalSaved.PointLauncherIndex, OriginalSaved.ShipID);
      ...
      4: //Move Selected Waypoint Long Lat
        begin
          ...
          begin
            ...
            if (PointBOLW.OwnerID = r^.PlatformID) and (PointBOLW.LauncherIndex = r^.BOLWLauncherIndex) and (PointBOLW.IndexPoint = r^.BOLWIndex) then
            begin
      ...
      8:
        begin
          ...
          case r^.PopUp of
            1:
              VehicleWeapon.HitMode.PopUp_Move := ppPopUP;
            2:
              VehicleWeapon.HitMode.PopUp_Move := ppSeaSkim;
          end;
        end;
      9:
        begin
      ...
      11:
        begin
          ...
          begin
            ...
            if (PointBOLW.IndexPoint = r^.BOLWIndex) and (PointBOLW.LauncherIndex = r^.BOLWLauncherIndex) then
            begin
              SimPointBOLW_VehicleMissile.deleteObject(PointBOLW);
              Break;
            end;
      ...
      begin
        ...
        begin
          ...
          case r^.MIssile_Method of
            Integer(mfmRBl):
              VehicleWeapon.FiringMode := mfmRBl;
            Integer(mfmRBLW):
              VehicleWeapon.FiringMode := mfmRBlW;
            Integer(mfmBOL):
              VehicleWeapon.FiringMode := mfmBOL;
            Integer(mfmBOLW):
              VehicleWeapon.FiringMode := mfmBOLW;
            Integer(mfmDirect):
              VehicleWeapon.FiringMode := mfmDirect;
            Integer(mfmAimpoint):
              begin
  ...
  begin
    ...
    begin
      ...
      case r^.cmd of
        0: //Add Waypoint
          begin
            PointRBLW := TPointVehicleMissileRBLW.Create(r^.RBLW_PointX, r^.RBLW_POintY, r^.RBLWIndex, r^.RBLWLauncherIndex, r^.PlatformID);
          ...
          begin
            ...
            begin
              ...
              case OriginalSaved.Action of
                waMove:
                  begin
                    ...
                    begin
                      ...
                      if (OriginalSaved.ShipID = r^.PlatformID) and (OriginalSaved.PointIndex = PointRBLW.IndexPoint) and (OriginalSaved.PointLauncherIndex = PointRBLW.LauncherIndex) then
                      begin
                ...
                waDelete:
                  begin
                    PointRBLW := TPointVehicleMissileRBLW.Create(OriginalSaved.PointX, OriginalSaved.PointY, OriginalSaved.PointIndex, OriginalSaved.PointLauncherIndex, OriginalSaved.ShipID);
        ...
        4: //Move Selected Waypoint Long Lat
          begin
            ...
            begin
              ...
              if (PointRBLW.OwnerID = r^.PlatformID) and (PointRBLW.LauncherIndex = r^.RBLWLauncherIndex) and (PointRBLW.IndexPoint = r^.RBLWIndex) then
              begin
        ...
        11:
          begin
            ...
            begin
              ...
              if (PointRBLW.IndexPoint = r^.RBLWIndex) and (PointRBLW.LauncherIndex = r^.RBLWLauncherIndex) then
              begin
                SimPointRBLW_VehicleMissile.deleteObject(PointRBLW);
                Break;
              end;
          ...
          case r^.Mode of
            ...
            //Add Waypoint Event
              ...
              //5 Weapon / 1 Coordinat Wayoint
                  ...
                  begin
                    ...
                    case i of
                      1:
                        TWaypointEventClass(WpEvent).FType := wetWeapon1;
                      2:
                        TWaypointEventClass(WpEvent).FType := wetWeapon2;
                      3:
                        TWaypointEventClass(WpEvent).FType := wetWeapon3;
                      4:
                        TWaypointEventClass(WpEvent).FType := wetWeapon4;
                      5:
                        TWaypointEventClass(WpEvent).FType := wetWeapon5;
                    end;
            ...
            7:
              begin
                case r^.Index of
                  0:
                    OwnVehicle.Waypoints.Termination := wtLH;
                  1:
                    OwnVehicle.Waypoints.Termination := wtTH;
                  2:
                    OwnVehicle.Waypoints.Termination := wtRP;
                  3:
                    OwnVehicle.Waypoints.Termination := wtRP2;
                  4:
                    OwnVehicle.Waypoints.Termination := wtRB;
                  5:
                    OwnVehicle.Waypoints.Termination := wtRM;
                  6:
                    OwnVehicle.Waypoints.Termination := wtSp;
                end;
...
//                    end;
      ...
      1: //Sensor : Radar, Sonar, IFF
        begin
          OwnVehicle.Waypoints.SensorStateChange(r^.Index, r^.List_Index, r^.WpEventType, r^.Mode);
      ...
      2: //Sonobuoy
        begin
          case r^.Mode of
            0:
              begin
                OwnVehicle.Waypoints.SonobuoyChange(r^.Index, r^.List_Index, r^.Sonobuoy_Index, r^.Device_ID);

                EventManager.OnWaypointDataChange(r^);
              end;
            1:
              begin
                OwnVehicle.Waypoints.SonobuoyModeChange(r^.Index, r^.List_Index, r^.Sonobuoy_Index, r^.Sonobuoy_Mode);

                EventManager.OnWaypointDataChange(r^);
              end;
            2:
              begin
                OwnVehicle.Waypoints.SonobuoyDepthChange(r^.Index, r^.List_Index, r^.Sonobuoy_Index, r^.Sonobuoy_Depth);

                EventManager.OnWaypointDataChange(r^);
              end;
            3:
              begin
                OwnVehicle.Waypoints.SonobuoySpacingChange(r^.Index, r^.List_Index, r^.Sonobuoy_Index, r^.Sonobuoy_Spacing);

                EventManager.OnWaypointDataChange(r^);
              end;
            4:
              begin
                OwnVehicle.Waypoints.SonobuoyQuantityChange(r^.Index, r^.List_Index, r^.Sonobuoy_Index, r^.Sonobuoy_Qty);

                EventManager.OnWaypointDataChange(r^);
              end;
            5:
              begin
                OwnVehicle.Waypoints.SonobuoyContinueStateChange(r^.Index, r^.List_Index, r^.Sonobuoy_Index, r^.Sonobuoy_ContinueDeploy);
      ...
      3: //Weapon : Missile, Torpedo, Bomb, Depth Charge
        begin
          case r^.Mode of
            0:
              begin
                OwnVehicle.Waypoints.WeaponChange(r^.Index, r^.List_Index, r^.Weapon_Type, r^.Weapon_ID);

                EventManager.OnWaypointDataChange(r^);
              end;
            1:
              begin
                OwnVehicle.Waypoints.WeaponSalvoChange(r^.Index, r^.List_Index, r^.Weapon_Type, r^.Salvo_Size);

                EventManager.OnWaypointDataChange(r^);
              end;
            2:
              begin
                OwnVehicle.Waypoints.WeaponTargetChange(r^.Index, r^.List_Index, r^.Weapon_ID, r^.Target_Index);
      ...
      4: //Mine
        begin
          case r^.Mode of
            0:
              begin
                OwnVehicle.Waypoints.MineChange(r^.Index, r^.List_Index, r^.Mine_Index, r^.Weapon_ID);

                EventManager.OnWaypointDataChange(r^);
              end;
            1:
              begin
                OwnVehicle.Waypoints.MineDepthChange(r^.Index, r^.List_Index, r^.Mine_Index, r^.Mine_Depth);

                EventManager.OnWaypointDataChange(r^);
              end;
            2:
              begin
                OwnVehicle.Waypoints.MineSpacingChange(r^.Index, r^.List_Index, r^.Mine_Index, r^.Mine_Spacing);

                EventManager.OnWaypointDataChange(r^);
              end;
            3:
              begin
                OwnVehicle.Waypoints.MineQuantityChange(r^.Index, r^.List_Index, r^.Mine_Index, r^.Mine_Qty);

                EventManager.OnWaypointDataChange(r^);
              end;
            4:
              begin
                OwnVehicle.Waypoints.MineContinueStateChange(r^.Index, r^.List_Index, r^.Mine_Index, r^.Mine_ContinueDeploy);
      ...
      5: //Countermeasure
        begin
          OwnVehicle.Waypoints.CountermeasureStateChange(r^.Index, r^.List_Index, r^.Mode);

          EventManager.OnWaypointDataChange(r^);
        end;
      6: //Datalink
        begin
          OwnVehicle.Waypoints.DatalinkStateChange(r^.Index, r^.List_Index, r^.WpEventType, r^.Mode);

          EventManager.OnWaypointDataChange(r^);
        end;
      7: //Continous Event
        begin
          OwnVehicle.Waypoints.ContinousEventChange(r^.Obj_Index, r^.Obj_Type, r^.Obj_Mode, r^.Obj_Depth, r^.Obj_Spacing, r^.Obj_Qty, r^.Obj_ContinueDeploy);
        end;
    end;
  end;
end;
...
//          NetSendUDP_MovementData(Obj);
      ...
      OnLogEventStr('TSimMgr_Server.netRecv_Synch_Pos_VBS', 'Sender : ' + LongIp_To_StrIp(r^.PID.ipSender) + 'Platform Name : ' + TT3PlatformInstance(Obj).InstanceName + ' ; Platform ID = ' + IntToStr(r^.PlatformID) + ' X = ' + X + ' Y = ' + Y + ' Z = ' + Z + ' Course = ' + Course + ' Speed = ' + FormatFloat('0.00', r^.Speed {* 0.539957}) + ' Altitude = ' + FormatFloat('0.00', r^.Z{C_Feet_To_Meter}));
    end;
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      IRCrossSection := TT3InfraredDecoy(Sender).InfraredDecoyDefinition.FInfraRedDecoy_Def.Max_Infrared_Cross * BloomDuration / TT3InfraredDecoy(Sender).BloomTime;
    ...
    begin
      DissipationTime := TT3InfraredDecoy(Sender).MaxDissipationTime - ((TT3InfraredDecoy(Sender).MaxDissipationTime - TT3InfraredDecoy(Sender).MinDissipationTime) * GameEnvironment.FData.Wind_Speed / 100);

      IRCrossSection := IRCrossSection - (TT3InfraredDecoy(Sender).MaxInfraredIntensity / DissipationTime * aDeltaMs);
    end;

    TT3InfraredDecoy(Sender).Altitude := TT3InfraredDecoy(Sender).Altitude - aDeltaMs * (TT3InfraredDecoy(Sender).DescentRate * C_Feet_To_Meter / 60);
...
function TT3SimManager.CreatePlatformInstance(piType, Index, NewId: integer): TPlatform_Instance;
var
  pi: TPlatform_Instance;
begin
  ...
  case piType of
    // vehicle
    1:
      pi.FData.Vehicle_Index := Index;
    2:
      pi.FData.Mine_Index := Index;
  end;
...
function TT3SimManager.CreatePlatformInstanceRP(piType, Index, NewId: integer): TPlatform_Instance;
var
  pi: TPlatform_Instance;
...
procedure TT3SimManager.CopyDataPlatformInstance(const piInput: TPlatform_Instance; var piOutput: TPlatform_Instance);
var
  i: Integer;
begin
  with piOutput do
...
function TT3SimManager.CreateTorpedoProjectileHybrid(const PlatformID, MisID, InstanceIndex: Integer): TObject;
var
  pi: TT3PlatformInstance;
...
function TT3SimManager.LaunchChaffBarrier(const rec: TRecCmd_LaunchChaff): TObject;
var
  pi: TT3PlatformInstance;
  cob{, lc}: TObject;
  chaff: TT3Chaff;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if (embarkHost.EmbarkedVehicle.Vehicle_Index = ve.VehicleDefinition.FData.Vehicle_Index) and (embarkHost.EmbarkedVehicle.Hosted_Vehicle_Index = embarkPlatform.VehicleDefinition.FData.Vehicle_Index) then
          begin
            found := True;
            Break;
          end;
...
//          embarkHost.Quantity := embarkHost.Quantity + 1;  //tambah quantity
         ...
         // hostRec.Quantity := hostRec.Quantity + 1;
          ...
          embarkHost.AddQueueLand(embarkPlatform.Track_ID, embarkPlatform.InstanceName, vEmbark.VehicleDefinition.FData.Readying_Time);
...
begin
  ...
  begin
    case r^.Envi_Chance of
      E_Wind_Speed:
        GameEnvironment.FData.Wind_Speed := r^.Value;
      E_Wind_Direction:
        GameEnvironment.FData.Wind_Direction := r^.Value;
      E_Daytime_Visual_Modifier:
        GameEnvironment.FData.Daytime_Visual_Modifier := r^.Value;
      E_Nighttime_Visual_Modifier:
        GameEnvironment.FData.Nighttime_Visual_Modifier := r^.Value;
      E_Daytime_Infrared_Modifier:
        GameEnvironment.FData.Daytime_Infrared_Modifier := r^.Value;
      E_Nighttime_Infrared_Modifier:
        GameEnvironment.FData.Nighttime_Infrared_Modifier := r^.Value;
      E_Sunrise:
        GameEnvironment.FData.Sunrise := Round(r^.Value);
      E_Sunset:
        GameEnvironment.FData.Sunset := Round(r^.Value);
      E_Period_of_Twilight:
        GameEnvironment.FData.Period_of_Twilight := Round(r^.Value);
      E_Rain_Rate:
        GameEnvironment.FData.Rain_Rate := Round(r^.Value);
      E_Cloud_Base_Height:
        GameEnvironment.FData.Cloud_Base_Height := r^.Value;
      E_Cloud_Attenuation:
        GameEnvironment.FData.Cloud_Attenuation := Round(r^.Value);
      E_Sea_State:
        GameEnvironment.FData.Sea_State := Round(r^.Value);
      E_Ocean_Current_Speed:
        GameEnvironment.FData.Ocean_Current_Speed := r^.Value;
      E_Ocean_Current_Direction:
        GameEnvironment.FData.Ocean_Current_Direction := r^.Value;
      E_Thermal_Layer_Depth:
        GameEnvironment.FData.Thermal_Layer_Depth := r^.Value;
      E_Sound_Velocity_Type:
        GameEnvironment.FData.Sound_Velocity_Type := Round(r^.Value);
      E_Surface_Sound_Speed:
        GameEnvironment.FData.Surface_Sound_Speed := r^.Value;
      E_Layer_Sound_Speed:
        GameEnvironment.FData.Layer_Sound_Speed := r^.Value;
      E_Bottom_Sound_Speed:
        GameEnvironment.FData.Bottom_Sound_Speed := r^.Value;
      E_Bottomloss_Coefficient:
        GameEnvironment.FData.Bottomloss_Coefficient := Round(r^.Value);
      E_Ave_Ocean_Depth:
        GameEnvironment.FData.Ave_Ocean_Depth := r^.Value;
      E_CZ_Active:
        GameEnvironment.FData.CZ_Active := Round(r^.Value);
      E_Surface_Ducting_Active:
        GameEnvironment.FData.Surface_Ducting_Active := Round(r^.Value);
      E_Upper_Limit_Surface_Duct_Depth:
        ;
      E_Lower_Limit_Surface_Duct_Depth:
        ;
      E_Sub_Ducting_Active:
        GameEnvironment.FData.Sub_Ducting_Active := Round(r^.Value);
      E_Upper_Limit_Sub_Duct_Depth:
        GameEnvironment.FData.Upper_Limit_Sub_Duct_Depth := r^.Value;
      E_Lower_Limit_Sub_Duct_Depth:
        GameEnvironment.FData.Lower_Limit_Sub_Duct_Depth := r^.Value;
      E_Shipping_Rate:
        GameEnvironment.FData.Shipping_Rate := Round(r^.Value);
      E_Shadow_Zone_Trans_Loss:
        GameEnvironment.FData.Shadow_Zone_Trans_Loss := r^.Value;
      E_Atmospheric_Refract_Modifier:
        GameEnvironment.FData.Atmospheric_Refract_Modifier := r^.Value;
      E_Barometric_Pressure:
        GameEnvironment.FData.Barometric_Pressure := r^.Value;
      E_Air_Temperature:
        GameEnvironment.FData.Air_Temperature := r^.Value;
      E_Surface_Temperature:
        GameEnvironment.FData.Surface_Temperature := r^.Value;
  ...
  begin
    ...
    begin
      case r^.Envi_Chance of
        E_Wind_Speed:
          sub.FData.Wind_Speed := r^.Value;
        E_Wind_Direction:
          sub.FData.Wind_Direction := r^.Value;
        E_Daytime_Visual_Modifier:
          sub.FData.Daytime_Visual_Modifier := r^.Value;
        E_Nighttime_Visual_Modifier:
          sub.FData.Nighttime_Visual_Modifier := r^.Value;
        E_Daytime_Infrared_Modifier:
          sub.FData.Daytime_Infrared_Modifier := r^.Value;
        E_Nighttime_Infrared_Modifier:
          sub.FData.Nighttime_Infrared_Modifier := r^.Value;
        E_Sunrise:
          ;
        E_Sunset:
          ;
        E_Period_of_Twilight:
          ;
        E_Rain_Rate:
          sub.FData.Rain_Rate := Round(r^.Value);
        E_Cloud_Base_Height:
          sub.FData.Cloud_Base_Height := r^.Value;
        E_Cloud_Attenuation:
          sub.FData.Cloud_Attenuation := Round(r^.Value);
        E_Sea_State:
          sub.FData.Sea_State := Round(r^.Value);
        E_Ocean_Current_Speed:
          sub.FData.Ocean_Current_Speed := r^.Value;
        E_Ocean_Current_Direction:
          sub.FData.Ocean_Current_Direction := r^.Value;
        E_Thermal_Layer_Depth:
          sub.FData.Thermal_Layer_Depth := r^.Value;
        E_Sound_Velocity_Type:
          sub.FData.Sound_Velocity_Type := Round(r^.Value);
        E_Surface_Sound_Speed:
          sub.FData.Surface_Sound_Speed := r^.Value;
        E_Layer_Sound_Speed:
          sub.FData.Layer_Sound_Speed := r^.Value;
        E_Bottom_Sound_Speed:
          sub.FData.Bottom_Sound_Speed := r^.Value;
        E_Bottomloss_Coefficient:
          sub.FData.Bottomloss_Coefficient := Round(r^.Value);
        E_Ave_Ocean_Depth:
          sub.FData.Ave_Ocean_Depth := r^.Value;
        E_CZ_Active:
          sub.FData.CZ_Active := Round(r^.Value);
        E_Surface_Ducting_Active:
          sub.FData.Surface_Ducting_Active := Round(r^.Value);
        E_Upper_Limit_Surface_Duct_Depth:
          sub.FData.Upper_Limit_Sur_Duct_Depth := r^.Value;
        E_Lower_Limit_Surface_Duct_Depth:
          sub.FData.Lower_Limit_Sur_Duct_Depth := r^.Value;
        E_Sub_Ducting_Active:
          sub.FData.Sub_Ducting_Active := Round(r^.Value);
        E_Upper_Limit_Sub_Duct_Depth:
          sub.FData.Upper_Limit_Sub_Duct_Depth := r^.Value;
        E_Lower_Limit_Sub_Duct_Depth:
          sub.FData.Lower_Limit_Sub_Duct_Depth := r^.Value;
        E_Shipping_Rate:
          sub.FData.Shipping_Rate := Round(r^.Value);
        E_Shadow_Zone_Trans_Loss:
          sub.FData.Shadow_Zone_Trans_Loss := r^.Value;
        E_Atmospheric_Refract_Modifier:
          sub.FData.Atmospheric_Refract_Modifier := r^.Value;
        E_Barometric_Pressure:
          sub.FData.Barometric_Pressure := r^.Value;
        E_Air_Temperature:
          sub.FData.Air_Temperature := r^.Value;
        E_Surface_Temperature:
          sub.FData.Surface_Temperature := r^.Value;
        E_Start_HF_Range_Gap:
          ;
        E_End_HF_Range_Gap:
          ;
...
function TT3SimManager.DisembarkTransportedPersonel(const rec: TRecCmd_DisembarkPersonelTemporary): TT3PlatformInstance;
var
...
procedure TT3SimManager.LauncherChaffProp(const rec: TRecCmd_ChaffLauncherProperties);
var
  pi: TT3PlatformInstance;
  lc: TObject;
begin
  ...
  case rec.Tag of
    1:
      TT3ChaffLauncher(lc).Angle := rec.Bearing;
    2:
      TT3ChaffLauncher(lc).BloomRange := rec.BloomRange;
    3:
      TT3ChaffLauncher(lc).BloomAltitude := rec.BloomAltitude;
    4:
      TT3ChaffLauncher(lc).SalvoSize := rec.SalvoSize;
    5:
      TT3ChaffLauncher(lc).Delay := rec.Delay;
  end;

  TT3ChaffLauncher(lc).IDState := rec.IDState;
end;

function TT3SimManager.LauncherChaffState(const rec: TRecCmd_LaunchChaffState): TObject;
var
  pi: TT3PlatformInstance;
  cob, lc: TObject;
begin
...
procedure TT3SimManager.LauncherBubbleProp(const rec: TRecCmd_BubbleLauncherProperties);
var
  pi: TT3PlatformInstance;
  lc: TObject;
begin
...
begin
  ...
  begin
    ...
    begin
      ...
      if projectile.UnitDefinition is TMine_On_Board then
      begin
      ...
      else if projectile.UnitDefinition is TMine_Definition then
      begin
...
begin
  ...
  begin
    ...
    if Assigned(SimPlatforms) then
      Sonobuoy.SimPlatforms := SimPlatforms;
...
function TT3SimManager.LauncherBubbleState(const rec: TRecCmd_LaunchBubbleState): TObject;
var
  pi: TT3PlatformInstance;
  cob: TObject;
begin
...
var
  ...
  Behav: TScripted_Behav_Definition;
  objList: TList;
  dist, dth, newX, newY, distanceW, fuelCons, fuelRmn, fuelCost, lubRmn, lubCost, freshWtrRmn, freshWtrCost, waterRmn, waterCost, foodRmn, foodCost, {fuelTrsAfterTime, MLTrsAfterTime, ATTrsAfterTime, waterTrsAfterTime,
  foodTrsAfterTime,} bearingW, speedw, altitudew, diswtemp, sDistanceW, bearingWtemp, sBearingW, startBearing, difwtemp, dthW, dthTemp: Double;
begin
              ...
              byte(entFuel):
                begin
                  ...
                  begin
                    ...
                    if (objv.PlatformType = 20) or (objv.PlatformType = 1) or (objv.PlatformType = 2) or (objv.PlatformType = 3) or (objv.PlatformType = 5) or (objv.PlatformType = 7) then
                      objv.Altitude := 0;
                  end;
                end;
            end;
            ...
            begin
              ...
              begin
                ...
                begin
                  ...
                  distanceW := CalcRange(objv.PosX, objv.PosY, Behav.FData.Waypoint_Longitude, Behav.FData.Waypoint_Latitude);
                  // bearing to waypoint j
                  bearingW := CalcBearing(objv.PosX, objv.PosY, Behav.FData.Waypoint_Longitude, Behav.FData.Waypoint_Latitude);
...
begin
  ...
  case rec.Index of
    // Boolean values
    GSID_AUTOREMOVEPF, GSID_RPTAIRBUBBLERMV..GSID_RPTSONOBUOYRMV, GSID_COASTLINECOLL, GSID_AIRCRAFTCOLLTERRAIN, GSID_SimplifiedSensor..GSID_IFFFilter, GSID_LandmassOcculting, GSID_ProcessSensorBZ, GSID_AcTorpedoDecoySetup..GSID_ProcessWeaponBZ:
    ...
    GSID_FLASHTransmission..GSID_RoutineTransmission, GSID_AcBearingDispLost..GSID_SurfaceLandDropTrack:

      tmp := SecondToTimeString(rec.ValInteger);

    // double values
      GSID_CubicalGroupClutter:
      tmp := FormatFloat('#.##', rec.ValDouble);

    // integer values
      GSID_VisualVisDetectF:
      tmp := IntToStr(rec.ValInteger);
  end;

  case rec.Index of
    GSID_RPTCHAFFRMV:
      GameSetting.ChaffRemovalEvent := rec.ValBoolean;
    GSID_RPTSONOBUOYRMV:
      GameSetting.SonobuoyRemovalEvent := rec.ValBoolean;
    GSID_RPTAIRBUBBLERMV:
      GameSetting.BubbleRemovalEvent := rec.ValBoolean;
    GSID_AUTOREMOVEPF:
      GameSetting.AutomaticRemovePlatformOutsideArea := rec.ValBoolean;
    GSID_PROBOFKILL:
      begin
        case rec.ValByte of
          0:
            GameSetting.ProbabilityOfKill := sNormal;
          1:
            GameSetting.ProbabilityOfKill := sAlways;
          2:
            GameSetting.ProbabilityOfKill := sNever;
        end;
        case rec.ValByte of
          0:
            tmp := 'Normal';
          1:
            tmp := 'Always kill if hit';
          2:
            tmp := 'Never kill';
        end;
      end;
    GSID_COASTLINECOLL:
      GameSetting.CoastlineCollision := rec.ValBoolean;
    GSID_AIRCRAFTCOLLTERRAIN:
      GameSetting.AircraftTerrainColl := rec.ValBoolean;
    GSID_TidalStreamEffects:
      GameSetting.TidalStreamEffectToShip := rec.ValBoolean;
    GSID_WindEffects:
      GameSetting.WindEffectToAircraft := rec.ValBoolean;
    GSID_GuidanceCommand:
      begin
        case rec.ValByte of
          0:
            GameSetting.GuidanceCommand := gcRealistic;
          1:
            GameSetting.GuidanceCommand := gcDirect;
        end;
        case rec.ValByte of
          0:
            tmp := 'Realistic';
    ...
    GSID_SimplifiedSensor:
      GameSetting.SimplifySensorCheck := rec.ValBoolean;
    GSID_LaunchRestrictions:
      GameSetting.StudentLaunchRestriction := rec.ValBoolean;
    GSID_CanLaunchAircraft:
      GameSetting.StudentCanLaunchAirCraft := rec.ValBoolean;
    ...
    GSID_InterfSoundEffects:
      ;
    GSID_AutomaticSensorId:
      GameSetting.AutomaticSensorIdentity := rec.ValBoolean;
    GSID_ChaffAttenuation:
      GameSetting.ChaffAttenuationOnRadar := rec.ValBoolean;
    ...
    GSID_IFFFilter:
      ;
    GSID_VisualVisDetectF:
      GameSetting.VisualVisibility := rec.ValInteger;
    GSID_LandmassOcculting:
      GameSetting.LandmassOcculting := rec.ValBoolean;
    ...
    GSID_CubicalGroupClutter:
      ;
    GSID_AcBearingDropTrack:
      GameSetting.AcousticTrackDropTime := rec.ValInteger;
    GSID_AirDropTrack:
      GameSetting.AirTrackDropTime := rec.ValInteger;
    GSID_ESMBearingDropTrack:
      GameSetting.ESMTrackDropTime := rec.ValInteger;
    GSID_SubsurfaceDropTrack:
      GameSetting.SubsurfaceTrackDropTime := rec.ValInteger;
    GSID_SurfaceLandDropTrack:
      GameSetting.SurfaceTrackDropTime := rec.ValInteger;
    GSID_AcBearingDispLost:
      GameSetting.AcousticTrackLostContactTimer := rec.ValInteger;
    GSID_AirDispLost:
      GameSetting.AirTrackLostContactTime := rec.ValInteger;
    GSID_ESMBearingDispLost:
      GameSetting.ESMTrackLostContactTime := rec.ValInteger;
    GSID_SubsurfaceDispLost:
      GameSetting.SubSurfaceTrackLostContatTime := rec.ValInteger;
    GSID_SurfaceLandDispLost:
      GameSetting.SurfaceTrackLostContactTime := rec.ValInteger;
    GSID_WRECKTIME:
      GameSetting.TimeRemoveWreck := rec.ValInteger;
  end;
...
//      tgt.isTransferFromShip := True;
    ...
    {$REGION ' Merubah Logistik dari instruktur '}
      begin
        ...
        case rec.ItemID of
          liFuel:
            ve.FuelRemaining := rec.vFuel;
          liLubricants:
            ve.MLRemaining := rec.vLubricants;
          liWater:
            ve.ATRemaining := rec.vWater;
          liFood:
            ve.FoodRemaining := rec.vFood;
        end;
      end;
    {$ENDREGION}
  end;
...
procedure TT3SimManager.ChangeSystemState(const rec: TRecCmd_Change_SystemState);
var
  ve: TT3Vehicle;
begin
  ve := FindT3PlatformByID(rec.PlatfomID) as TT3Vehicle;
...
procedure TT3SimManager.GunSet(aList: TList; aClass: TClass; aParent: TT3Vehicle);
var
  j: integer;
  weapon: TObject;
begin
  ...
  begin
    ...
    if aClass = TT3MissilesOnVehicle then
      weapon := TT3MissilesOnVehicle.Create
    else if aClass = TT3TorpedoesOnVehicle then
      weapon := TT3TorpedoesOnVehicle.Create
    else if aClass = TT3BombONVehicle then
      weapon := TT3BombONVehicle.Create
    else if aClass = TT3MineOnVehicle then
      weapon := TT3MineOnVehicle.Create
    else if aClass = TT3GunOnVehicle then
      weapon := TT3GunOnVehicle.Create
    else if aClass = TT3HybridOnVehicle then
      weapon := TT3HybridOnVehicle.Create;
...
function TT3SimManager.LaunchRuntimePlatform(const rec: TRecCmd_LaunchRP): TT3PlatformInstance;
var
  i: Integer;
...
begin
  ...
  begin
    ...
    else if rec.PSpeed < FDeployedPlatform.UnitMotion.FData.Min_Ground_Speed then
    begin
    ...
    else if (FDeployedPlatform.PlatformDomain = 2) then
    begin
...
//      FActivation.Init_Logistic           := 100;
    ...
    if rec.PointType = nrpBearing then
      ...
      TT3NonRealVehicle(FDeployedObject).OrderedHeading := rec.Course;
    ...
    if Assigned(FOnLogInitStr) then
      FOnLogInitStr(IntToStr(TT3PlatformInstance(FDeployedObject).InstanceIndex) + ' : ' + FDeployedObject.InstanceName);
  end;
end;
...
procedure TT3SimManager.SensorAssets(pi: TPlatform_Instance; vehicle: TT3Vehicle);
begin
  ...
  case cmd.SensorType of
        ...
        case cmd.OrderID of
          ...
          CORD_ID_ControlRangeSonar:
            begin
              ...
              if Assigned(OnLogEventStr) then
                OnLogEventStr('TT3SimManager.SensorCommand', 'Sensor ' + TT3Sensor(obj).InstanceName + ', TIOW : ' + SonarTIOWRangeToString(TSonarTIOWRange(cmd.OrderParam)));
            end;
        end;
      ...
      begin
        ...
        case cmd.OrderID of
          CORD_ID_OperationalStatus:
            TT3ESMSensor(obj).OperationalStatus := TSensorOperationalStatus(cmd.OrderParam);
        end;
      end;
    ...
    CSENSOR_TYPE_EO:
      begin
        ...
        case cmd.OrderID of
          CORD_ID_OperationalStatus:
            TT3EOSensor(obj).OperationalStatus := TSensorOperationalStatus(cmd.OrderParam);
        end;
      end;
    ...
    CSENSOR_TYPE_MAD:
      begin
        ...
        case cmd.OrderID of
          CORD_ID_OperationalStatus:
            TT3MADSensor(obj).OperationalStatus := TSensorOperationalStatus(cmd.OrderParam);
        end;
      end;
    ...
    CSENSOR_TYPE_IFF:
      begin
        ...
        case cmd.OrderID of
          1:
            TT3IFFSensor(obj).TransponderOperateStatus := TSensorOperationalStatus(cmd.OrderParam);
          2:
            TT3IFFSensor(obj).InterrogatorOperateStatus := TSensorOperationalStatus(cmd.OrderParam);
        end;
      end;
  end;
...
procedure TT3SimManager.SensorSet(aList: TList; aClass: TClass; aParent: TT3Vehicle);
var
  j: integer;
  sensor: TObject;
begin
  ...
  begin
    ...
    if aClass = TT3Radar then
      sensor := TT3Radar.Create
    else if aClass = TT3Sonar then
      sensor := TT3Sonar.Create
    else if aClass = TT3Visual then
      sensor := TT3Visual.Create
    else if aClass = TT3EOSensor then
      sensor := TT3EOSensor.Create
    else if aClass = TT3ESMSensor then
      sensor := TT3ESMSensor.Create
    else if aClass = TT3MADSensor then
      sensor := TT3MADSensor.Create
    else if aClass = TT3FCRSensor then
      sensor := TT3FCRSensor.Create
    else if aClass = TT3IFFSensor then
      sensor := TT3IFFSensor.Create
    else if aClass = TT3SonobuoyOnVehicle then
      sensor := TT3SonobuoyOnVehicle.Create;
...
begin
  ...
  begin
    ...
    if Assigned(OnLogStr) then
      OnLogStr('CreateNonRealPlatformInstance', 'Default TPlatform_Instance for Non Real Platform = ' + FScenario.DefaultNonRealPlatform.Vehicle.FData.Vehicle_Identifier);
  end
  else
  begin
    if Assigned(OnLogStr) then
...
procedure TT3SimManager.CreateOverlayShapeDynamic(rec: TRecCmd_OverlayDynamicShape);
var
  i, j: Integer;
...
begin
  ...
  begin
    case rec.IdAction of
      ...
      caDelete:
      begin
        OverlayTemplate.DynamicList.Delete(rec.IdSelectShape);
      end;
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if device is TT3IFFSensor then
        begin
